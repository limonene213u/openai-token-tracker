...
// レスポンスの "usage" 情報をログ（token_usage_log.json）に追記する関数
void log_token_usage(const char *json_response) {
    cJSON *root = cJSON_Parse(json_response);
    if (!root) {
...
//
// --- main ---
// ここでは openai.json から設定を読み込み、
// ユーザーが "show_tokens" と入力するとログ集計を行い、
// それ以外の場合は API へのリクエストを行って応答を表示し、
// レスポンスのトークン使用量をログに記録します。
//
...
int main(void) {
    ...
    while (1) {
        ...
        // cJSON を使ってリクエストの JSON オブジェクトを構築
        ...
        if (curl) {
            ...
            if (res != CURLE_OK) {
                ...
                // レスポンスからトークン使用量をログに記録
                log_token_usage(chunk.response);
            }
